use ast::{Declaration, Expression, Stmt, VarType, ast::{Block, BlockElement, ForInit, FuncDecl, Program, VariableDecl}};
...
impl TypeChecker for SemanticAnalyzer {
    ...
    fn check_types_var_decl(&mut self, decl: &VariableDecl)->Result<(),SemanticError> {
        if !self.symbol_table.contains_key(&decl.name) {
                    self.symbol_table.insert(decl.name.clone(),SymbolEntry::Variable { 
    ...
    fn check_types_expr(&mut self,expr:&Expression)->Result<(),SemanticError> {
        match expr {
            Expression::FunctionCall { func_name, args }=>{
                 ...
                 if f_type.is_none() {
                    return Err(SemanticError::UndeclaredFunction { func_name: func_name.clone() });
                }
                 match f_type.unwrap(){
                     SymbolEntry::Function { var_type, .. }=>{
                         
                         if *var_type == VarType::Int {
                             return Err(SemanticError::VariableAsFunctionName { var_name: func_name.clone() });
                         }
            ...
            Expression::Var(name)=>{
                 if self.symbol_table.contains_key(name) { 
                    if self.symbol_table.get(name).unwrap().get_var_type()!= VarType::Int {
                      return Err(SemanticError::FunctionAsVariableName { func_name: name.clone() });
                    }
                 } else {
                    return Err(SemanticError::UndeclaredVariable { var_name: name.clone() });
                 }
             }
    ...
    fn register_func_decl(&mut self, decl: &FuncDecl)->Result<(),SemanticError> {
        ...
        if self.symbol_table.contains_key(&decl.name) {
            ...
            match old_entry{
                SymbolEntry::Function { already_defined: defined,var_type: old_type }=>{
        ...
        self.symbol_table.insert(decl.name.clone(),SymbolEntry::Function{ 
            var_type: fun_type,
    ...
    fn check_func_body(&mut self, decl: &FuncDecl)->Result<(),SemanticError> {
       if decl.body.is_some() {
            ...
            for param in decl.params.iter() {
                function_scope.insert(param.clone(),SymbolEntry::Variable { 
                    var_type: VarType::Int 
                });
            }
